...
8. Reconstruct encrypted bytes from any `k` valid shards.
9. Decrypt locally with the client-held secret.
...
The current code proves three early invariants: data survives shard loss up to the configured parity budget, online peers can be challenged for signed sampled possession of encrypted shards, and failed placements can be repaired without exposing plaintext or client keys. The next step is to replace the local swarm with real transport, durable shard storage, churn-aware scheduling, and adversarial audit randomness.
